...
out(f"Напряжение: {voltage} В")

# 2. Использование спецификаторов формата (например, .1f) внутри f-строки
out(f"Мощность: {voltage * current:.1f} Вт")

# 3. Передача нескольких f-строк для заполнения разных колонок
out(f"U = {voltage} V", f"I = {current} A")
_end_
# > Напряжение: 220.5 В
# > Мощность: 3307.5 Вт
# > U = 220.5 V I = 15 A
...
# > ---------------------------------------------
# > Температура    OK             85.5           °C             
# > Давление       ALARM          12.4           Бар            
# > Расход         OK             1040           м3/ч           
...
_end_
# > НАИМЕНОВАНИЕ        КОД       СТАТУС         
# > ----------------------------------------
# > Главный двигатель   M-01      В работе       
# > Насос охлаждения    P-12      Резерв         
# > Вентилятор          V-05      Ремонт         

_beg_
voltage = 220
current = 5.5
power = voltage * current
# Передача текстовых литералов и вычисленных переменных 
# явным указанием ширины колонок
...
import sys
sys.version ? 3.14.3 (tags/v3.14.3:323c59a, Feb  3 2026, 16:04:56) [MSC v.1944 64 bit (AMD64)]

# 2. Модуль для работы с датой и временем
import datetime
current_year = datetime.date.today().year ? 2026

# 3. Модуль статистики (удобно для экспресс-анализа массивов данных)
import statistics
data = [10, 15, 20, 25]
avg = statistics.mean(data) ? 17.500000

# 4. Генератор случайных чисел
import random
lucky_number = random.randint(1, 100) ? 67
_end_
```
...
# 5. Распаковка массивов (Unpacking)
source = [100, 200, 300, 400, 500]
head, *middle, tail = source
head ? 100
middle ? [200, 300, 400]
_end_
```
...
map1.update({'sensor_3': 25.0}) or map1 ? {'sensor_1': 22.5, 'sensor_2': 24.0, 'sensor_3': 25.0}

# 3. Удаление элемента с извлечением его значения
deleted_val = map1.pop('sensor_1') ? 22.500000

# 4. Объединение двух словарей в один новый (для Python 3.9+)
d_left = {'a': 1, 'b': 2}
d_right = {'b': 10, 'c': 3}
merged = d_left | d_right ? {'a': 1, 'b': 10, 'c': 3}
...
unique_data = set(raw_data) ? {40, 10, 20, 30}
...
## 10. f-строки и форматирование (Красивый вывод и таблицы)
...
radius = 10
info = f"Площадь: {pi * radius**2:.2f} см2" ? Площадь: 314.16 см2

# 2. Вывод одного числа в разных системах счисления (Dec, Hex, Bin)
val = 255
bases = f"Dec: {val}, Hex: 0x{val:02X}, Bin: 0b{val:08b}" ? Dec: 255, Hex: 0xFF, Bin: 0b11111111

# 3. Выравнивание строк для создания ровных столбцов таблиц
# :<10 (по левому краю), :>10 (по правому краю), :^10 (по центру)
v1, v2 = 10.5, 100.25
line1 = f"Параметр А: {v1:>10}" ? Параметр А:       10.5
line2 = f"Параметр B: {v2:>10}" ? Параметр B:     100.25
...
v1 = [1, 2, 3]
v2 = [4, 5, 6]
# Скалярное произведение: последовательно перемножаем 
# пары элементов (zip) и суммируем результат
...
# Шаг А: Разбиваем строку на пары ['Имя', 'Значение'], удаляя 
# лишние пробелы через strip()
...
clean_data ? {'T1': 22.5, 'T3': 24.1, 'T4': 19.8}
...
# (Конструкция list() необходима, чтобы развернуть итератор в видимый текстовый список)
...
# 3. Цепочка (Pipeline): Сначала фильтруем ошибки, 
#    затем результат сразу переводим в числа
final = list(map(float, filter(lambda x: x != "err", data))) ? [10.5, 20.1, 30.5]
_end_
```

## 5. Работа с несколькими списками одновременно (zip)
...
value = 2.675
rounded_bank = float(Decimal(str(value)).quantize(Decimal('0.01'), rounding=ROUND_HALF_EVEN)) ? 2.68
rounded_std = round(value, 2) ? 2.67

# 3. Статистические характеристики массива 
#    (Прямой расчет базовых формул без импорта модуля statistics)
data = [12, 15, 18, 21, 24]
n = len(data)
mean_val = sum(data) / n ? 18
variance = sum((x - mean_val) ** 2 for x in data) / (n - 1) ?.1 22.5
std_dev = variance ** 0.5 ?.2 4.74
_end_
```
...
# 2. Цепочка безопасного доступа к глубоко вложенным словарям через тернарный оператор
...
# 3. Валидация входных данных с защитой от пустых значений типа None и несоответствия типов
process = lambda x: round(x ** 2, 2) if x is not None and isinstance(x, (int, float)) else 0
result1 = process(5.5) ? 30.25
...
# 2. Реализация паттерна "Pipeline" (Последовательный 
#    конвейер функций) с помощью reduce
from functools import reduce
...
# 3. Логирование промежуточных состояний конвейера 
#    в процессе расчета через out()
data_flow = [1, 2, 3, 4, 5]
