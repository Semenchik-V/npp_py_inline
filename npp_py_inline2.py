...
def show_smart_help(query, calc_scope, mode='_help_'):
    import types
    # Исправлен способ получения пути скрипта (совместимость с PythonScript)
    try:
    ...
    if mode == '_man_':
        ...
        if not os.path.exists(path):
            try:
                ...
                err_content = "=== МОЙ ИНЖЕНЕРНЫЙ СПРАВОЧНИК ===\n# [ВНИМАНИЕ: Ошибка I/O, файл не сохранен на диск: {}]\n".format(e)
    ...
    if isinstance(target, types.ModuleType):
        ...
        for name in sorted(calc_scope.keys()):
            ...
            if callable(obj) and not name.startswith("_"):
                ...
                content.append("{:15} - {}".format(name, doc))
        open_tab("SmartRef.txt", "\n".join(content))
...
class MathEngine:
    ...
    def build_row(self, *args, **kwargs):
        """
        ...
        line = "".join(["{:<{}}".format(str(arg), width) for arg, width in zip(args, widths)])
    ...
    def execute_assignment(self, var_part, result):
        ...
        exec("{} = __sc_res__".format(var_part), self.scope, self.scope)
...
class CodeParser:
    ...
    def parse(full_raw_line):
        ...
        if match:
            ...
            try:
                compile("{} = None".format(left_candidate), "<string>", "exec")
    ...
    def format(parsed, result, actual_formula, engine):
        ...
        if result is None: 
            ...
            if is_effectively_integer(result):
                ...
                res_str = "{:.{}f}".format(result, precision)
            # === КОНЕЦ ИЗМЕНЕНИЙ ===
        ...
        base_expr = "{} = {}".format(parsed['var_part'], actual_formula) if parsed['var_part'] else actual_formula

        # Применение режимов Clean и Debug
        if clean_mode:
            if parsed['is_inspector']: 
                return "{} ?{}".format(base_expr, flag_str).strip()
            ...
            if parsed['is_inspector']:
                if res_str:
                    return "{} ?{} {}".format(base_expr, flag_str, res_str).strip()
                else:
                    return "{} ?{}".format(base_expr, flag_str).strip()
            if debug_mode and res_str and str(result) != actual_formula.strip():
                return "{} = {}".format(base_expr, res_str)
            return base_expr
